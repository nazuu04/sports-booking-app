...
if(isset($_POST['update'])){
    ...
    mysqli_query($conn,"UPDATE lapangan SET nama_lapangan='$nama',jenis_olahraga='$jenis',harga_per_jam='$harga' WHERE id_lapangan='$id'");
    header("Location: lapangan.php"); exit;
}
?>
<!DOCTYPE html>
<html>
...
<div class="main">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-0" style="color:#e0ffe8">Edit Lapangan</h4>
            <small style="color:#2a4030">Ubah data lapangan</small>
        </div>
        <div class="avatar">A</div>
    </div>
    <div class="card-form">
        <div class="info-badge mb-4"><i class="ti ti-info-circle"></i> Mengubah data lapangan #<?= $id ?></div>
        <form method="POST">
            <div class="mb-3">
                <label class="form-label">Nama Lapangan</label>
                <div class="input-wrap"><i class="ti ti-building-stadium"></i><input type="text" name="nama" class="form-control" value="<?= $d['nama_lapangan'] ?>" required></div>
            </div>
            <div class="mb-3">
                <label class="form-label" style="margin-bottom:10px">Jenis Olahraga</label>
                <input type="hidden" name="jenis" id="jenis_input" value="<?= $d['jenis_olahraga'] ?>">
                <div class="jenis-grid">
                    <div class="jenis-card <?= $d['jenis_olahraga']=='Futsal'?'selected':'' ?>" onclick="pilihJenis(this,'Futsal')"><i class="ti ti-ball-football"></i><span>Futsal</span></div>
                    <div class="jenis-card <?= $d['jenis_olahraga']=='Badminton'?'selected':'' ?>" onclick="pilihJenis(this,'Badminton')"><i class="ti ti-feather"></i><span>Badminton</span></div>
            ...
            <div class="mb-4">
                ...
                <div class="input-wrap"><i class="ti ti-cash"></i><input type="number" name="harga" class="form-control" value="<?= $d['harga_per_jam'] ?>" required></div>
            </div>
