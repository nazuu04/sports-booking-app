...
<div class="main">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-0" style="color:#e0ffe8">Data Lapangan</h4>
            <small style="color:#2a4030">Kelola semua lapangan olahraga</small>
        </div>
        <div class="avatar">A</div>
    </div>
    <div class="mb-3"><a href="tambah.php" class="btn-tambah"><i class="ti ti-plus"></i> Tambah Lapangan</a></div>
    <table>
        <thead>
            <tr><th>ID</th><th>Nama Lapangan</th><th>Jenis</th><th>Harga/Jam</th><th>Aksi</th></tr>
        </thead>
        <tbody>
            <?php while($d = mysqli_fetch_array($data)): ?>
            <tr>
                <td style="color:#1a3020"><?= $d['id_lapangan'] ?></td>
                <td><?= $d['nama_lapangan'] ?></td>
                <td style="color:#5DCAA5"><?= $d['jenis_olahraga'] ?></td>
                <td style="color:#1D9E75">Rp <?= number_format($d['harga_per_jam'],0,',','.') ?></td>
                <td>
                    <a href="edit_lapangan.php?id=<?= $d['id_lapangan'] ?>" class="btn-edit"><i class="ti ti-edit"></i> Edit</a>
                    <a href="hapus_lapangan.php?id=<?= $d['id_lapangan'] ?>" class="btn-hapus" onclick="return confirm('Yakin hapus?')"><i class="ti ti-trash"></i> Hapus</a>
                </td>
            </tr>
            <?php endwhile; ?>
        </tbody>
